	...
	testTable := []struct {
	...
	}{
		{
			...
			mockBehavior: func(s *mock_service.MockToken, token string) {
				s.EXPECT().ParseToken(token).Return(&service2.CustomClaims{UserID: "1"}, nil)
			},
		...
		},
		{
			...
			token:       "token",
			mockBehavior: func(s *mock_service.MockToken, token string) {
				s.EXPECT().ParseToken(token).Return(&service2.CustomClaims{UserID: "1"},
					apperror.Authorization.New(service2.ErrorMessageInvalidToken))
			},
			expectedStatusCode:   401,
